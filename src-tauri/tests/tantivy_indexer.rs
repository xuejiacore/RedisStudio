...
async fn test() -> Result<(), TantivyError> {
    let path = PathBuf::from("/Users/nigel/Library/Application Support/org.nigel.redisstudio/tantivy_index");
    let indexer = TantivyIndexer::init(path).init_indexer().await;
    let write = true;
    if write {
        indexer::redis_indexer::index_or_update("testdatasource0", "130:commodity:32156", &indexer).await.unwrap();
        indexer::redis_indexer::index_or_update("testdatasource0", "131:commodity:32157", &indexer).await.unwrap();
        indexer::redis_indexer::index_or_update("testdatasource0", "133:commodity:32153", &indexer).await.unwrap();
        indexer::redis_indexer::index_or_update("testdatasource4", r"[a-zA-Z0-9]+:commodity:\d+", &indexer).await.unwrap();
    }
    ...
    let result = indexer::redis_indexer::match_pattern("datasource", ":commodity:", &indexer).await.unwrap();
    println!("{}", result);
