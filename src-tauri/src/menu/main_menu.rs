...
pub async fn process_main_menu(window: &Window, event: MenuEvent) {
    ...
    match menu_group {
        ...
        menu::MENU_OPERATOR_MENU => process_type_operator(window, event, menu_id, context, menu_id_str).await,
...
fn process_add_new_key(window: &Window, event: MenuEvent, menu_id: &str, context: HashMap<String, String>) {
    ...
    win.eval(format!("window.onCreateNewKey('{}', '{}', {})", key_type, datasource, database).as_str()).unwrap();
    win.show().unwrap();
}

/// type operator
async fn process_type_operator(window: &Window, event: MenuEvent, menu_id: &str, context: HashMap<String, String>, menu_id_val: &str) {
    ...
    match menu_id_val {
        menu::MID_KEY_OP_ADD_ROW => {
            ...
            window.emit_to(win_label, "operator/add_row", payload).unwrap();
        }
        menu::MID_KEY_OP_DELETE => {
            ...
            let del_result: i32 = cmd("HDEL").arg(key).arg(field).query_async(conn.deref_mut()).await.expect("");
            ...
            window.emit_to(win_label, "operator/del_row", payload).expect("Fail to emit msg.");
        }
        &_ => {}
    }
}
...
async fn process_key_tree_right_clk(window: &Window, event: MenuEvent, menu_id: &str, context: HashMap<String, String>, menu_id_val: &str) {
    ...
    let database = context.get("database").expect("Parameter error: missing `dastabase`");
    let mut keys = vec![];
    ...
    let redis_pool: State<'_, RedisPool> = window.state();
    let t = redis_pool.select_connection(datasource, Some(database_num)).await;
    let mut conn = {
        let mutex = t.lock().await;
        mutex
    };
    ...
    for key in keys {
        match menu_id_val {
            ...
            menu::MID_KEY_RENAME => {
                ...
                win.eval(format!("window.onKeyModify('{}', '{}', '{}', {}, 'modify')", key, type_str, datasource, database).as_str()).unwrap();
                win.show().unwrap();
            }
            menu::MID_DUPLICATE => {
                let type_str: String = cmd("TYPE").arg(key).query_async(conn.deref_mut()).await.expect("Key not exists.");
                let win = window.get_webview_window("modify-key-win").unwrap();
                win.eval(format!("window.onKeyModify('{}', '{}', '{}', {}, 'duplicate')", key, type_str, datasource, database).as_str()).unwrap();
                win.show().unwrap();
            }
            &_ => todo!()
        }
