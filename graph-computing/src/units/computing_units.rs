...
use crate::row::proto_row::{DataType, Row};
...
impl UnitConfig {
    pub fn new(lua_source: &str) -> Self {
        UnitConfig { lua_source: lua_source.to_string() }
    }

    pub fn get_lua_source(&self) -> String {
        self.lua_source.clone()
    }
}
...
impl Default for UnitConfig {
    fn default() -> Self {
        UnitConfig {
            lua_source: Default::default()
        }
    }
}

/// Only a unit marker trait
pub trait Unit {}

/// Lifecycle for computing unit.
///
/// # Examples
///
/// ```
...
pub trait UnitLifecycle {
    /// This function will be invoked when this computing unit on initializing.
    fn init(&mut self, ctx: UnitContext);
    /// This function will be invoked when this computing unit on started.
    fn start(&self) -> core::result::Result<(), Box<dyn std::error::Error>> {
        Ok(())
    }
    /// This function will be invoked when this computing unit on finished.
    fn finish(&self, ctx: &UnitContext) {}
    /// This function will be invoked when this computing unit had exception.
    fn on_error(&self, ctx: &UnitContext) {}
    /// This function will be invoked when this computing unit was canceled.
    fn cancel(&self, ctx: &UnitContext) {}

    fn get_context(&self) -> core::result::Result<&UnitContext, ()> {
        Err(())
    }
}
...
impl ContextToolKit for BaseUnit {}

impl BaseUnit {
    pub(crate) fn new() -> Self {
        BaseUnit {
            ctx: Box::new(*UnitContext::new())
        }
    }

    pub(crate) fn start(&self) {
        println!("default source unit");
    }

    pub(crate) fn init(&mut self, ctx: UnitContext) {
        self.ctx = Box::new(ctx);
    }

    pub(crate) fn subscribe(&self, ch: &Box<RuntimeChannel>, consumer: fn(&Row)) {
        self.ctx.channel().subscribe(ch, consumer);
    }

    pub fn ctx(&self) -> &Box<UnitContext> {
        &self.ctx
    }
}
...
impl Default for BaseUnit {
    fn default() -> Self {
        BaseUnit { ctx: Box::new(*UnitContext::new()) }
    }
}

/// rust native computing unit trait
pub trait NativeUnit: Unit {
    /// calculate data, process input and generate output data
    fn calc(&self, row: Row, ctx: &UnitContext);
}

/// Lua supported unit trait
pub trait LuaUnit: Unit {
    /// calc from input row data, transfer value to lua table.
    fn calc(&self, row: Row, ctx: &UnitContext) {
        let lua_source = self.load_lua_source();
        let lua = Lua::new();
        lua.context(|lua_ctx| -> Result<()>{
            let globals = lua_ctx.globals();

            // todo: init rust native function

            let map_table = lua_ctx.create_table()?;
            for column in row.columns {
                match column.dt.unwrap() {
                    DataType::i32 => {
                        map_table.set(column.field, column.i32_val)?;
                    }
                    DataType::i64 => {
                        map_table.set(column.field, column.i64_val)?;
                    }
                    DataType::f64 => {
                        map_table.set(column.field, column.f64_val)?;
                    }
                    DataType::str => {
                        map_table.set(column.field, column.str_val)?;
                    }
                    DataType::unknown => {
                        map_table.set(column.field, column.str_val)?;
                    }
            ...
            globals.set("row", map_table)?;
            lua_ctx.load(lua_source.as_str()).exec().unwrap();
            Ok(())
